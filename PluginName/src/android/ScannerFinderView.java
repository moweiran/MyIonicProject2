...
public final class ScannerFinderView extends RelativeLayout {
    ...
    public ScannerFinderView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mMaskColor = resources
                .getColor(context.getResources().getIdentifier("finder_mask", "color", context.getPackageName()));
        mFrameColor = resources
                .getColor(context.getResources().getIdentifier("finder_frame", "color", context.getPackageName()));
        mLaserColor = resources
                .getColor(context.getResources().getIdentifier("finder_laser", "color", context.getPackageName()));
        mTextColor = resources
                .getColor(context.getResources().getIdentifier("white", "color", context.getPackageName()));
    ...
    // float left = (ScreenUtils.getScreenWidth() - mPaint.getTextSize() *
    // text.length()) / 2;
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        try {
                            ...
                            if (lastX >= 0) {
                                ...
                                boolean currentXRight = currentX >= rect.right - BUFFER
                                ...
                                boolean currentYBottom = currentY <= rect.bottom + BUFFER
    ...
    private void updateBoxRect(int dW, int dH, boolean isUpward) {
        ...
        int newWidth = (mFrameRect.width() + dW > ScrRes.x - 4 || mFrameRect.width() + dW < MIN_FOCUS_BOX_WIDTH) ? 0
                : mFrameRect.width() + dW;

        // 限制扫描框最大高度不超过屏幕宽度
        int newHeight = (mFrameRect.height() + dH > ScrRes.x || mFrameRect.height() + dH < MIN_FOCUS_BOX_HEIGHT) ? 0
                : mFrameRect.height() + dH;
