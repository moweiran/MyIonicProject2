...
public class FocusBoxView extends View {
    ...
    public FocusBoxView(Context context, AttributeSet attrs) {
        ...
        int r_color_focus_box_mask = TessocrScanner.appContext.getResources().getIdentifier("focus_box_mask", "color",
                packageName);
        int r_color_focus_box_frame = TessocrScanner.appContext.getResources().getIdentifier("focus_box_frame", "color",
                packageName);
        int r_color_focus_box_corner = TessocrScanner.appContext.getResources().getIdentifier("focus_box_corner",
                "color", packageName);

        maskColor = resources.getColor(r_color_focus_box_mask);
        frameColor = resources.getColor(r_color_focus_box_frame);
        cornerColor = resources.getColor(r_color_focus_box_corner);
    ...
    private void updateBoxRect(int dW, int dH) {
        ...
        int newWidth = (box.width() + dW > ScrRes.x - 4 || box.width() + dW < MIN_FOCUS_BOX_WIDTH) ? 0
                : box.width() + dW;

        int newHeight = (box.height() + dH > ScrRes.y - 4 || box.height() + dH < MIN_FOCUS_BOX_HEIGHT) ? 0
                : box.height() + dH;
